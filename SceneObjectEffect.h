...
#include <algorithm>
#include <iostream>
#include <fstream> // File output for logging. 
/**
...
public:
	...
	Vector3f rayTrace(Ray &ray, const float& t, const Vector3f& normal, RayTracerState& state) 
	{
		...
		if (  dot  < 0.0f )
		{
			...
			out_color = Math::mix( refract, reflect, Rf);  // Refraction - current
...
class CubeFresnelEffect : public SceneObjectEffect {
public:
	CubeFresnelEffect()
	:	out_color(0.0f)
	,	eta_air( 1.000293 )
	...
	Vector3f rayTrace(Ray &ray, const float& t, const Vector3f& normal, RayTracerState& state) 
	{
		...
		if (  dot  < 0.0f )
		{
			...
			// Check for total inner refraction
			//if ( refractDir.x == 0.0f && refractDir.y == 0.0f && refractDir.z == 0 ) return reflect;
			//std::cout << "Dir : " << dir_n << std::endl;
			//std::cout << "Ref : " << refractDir << std::endl;
